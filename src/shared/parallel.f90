...
! synchronizes all MPI processes across simultaneous runs (MPI_COMM_WORLD)
  ...
  call MPI_BARRIER(MPI_COMM_WORLD, ier)
  if (ier /= 0) stop 'Error synchronize_all_world'
...
!
  ...
  call MPI_BCAST(buffer,1,MPI_INTEGER8,0,my_local_mpi_comm_world,ier)
...
!
  ...
  !> Abort if ranks span multiple hostnames (arrival-order v1 is single-node only).
  ...
  interface
    function aos_get_hostname(buf, buflen) bind(C, name="aos_get_hostname")
      use, intrinsic :: iso_c_binding
      integer(c_int) :: aos_get_hostname
      character(kind=c_char), intent(out) :: buf(*)
      integer(c_int), value :: buflen
    end function aos_get_hostname
  end interface

  character(kind=c_char), dimension(128) :: c_host
  character(len=128) :: myhost, other
  character(len=128), allocatable :: allhosts(:)
  integer :: ier, rank, nprocs, i, j, k, rc

  call MPI_COMM_RANK(MPI_COMM_WORLD, rank, ier)
  call MPI_COMM_SIZE(MPI_COMM_WORLD, nprocs, ier)

  c_host = c_null_char
  rc = aos_get_hostname(c_host, 128_c_int)
  myhost = ''
  if (rc == 0) then
    do k = 1, 128
      if (c_host(k) == c_null_char) exit
      myhost(k:k) = c_host(k)
    enddo
  endif
  myhost = adjustl(myhost)

  allocate(allhosts(0:nprocs-1))
  call MPI_ALLGATHER(myhost, 128, MPI_CHARACTER, allhosts, 128, MPI_CHARACTER, &
                     MPI_COMM_WORLD, ier)
  if (ier /= 0) stop 'Error MPI_ALLGATHER in world_require_single_hostname'

  do i = 0, nprocs-1
    other = adjustl(allhosts(i))
    if (trim(other) /= trim(myhost)) then
      if (rank == 0) then
        write(*,*) 'Arrival-order scheduler requires all simulation groups on one host.'
        write(*,*) 'Detected hosts:'
        do j = 0, nprocs-1
          write(*,'(a,i6,a,a)') '  world_rank ', j, ': ', trim(adjustl(allhosts(j)))
        enddo
      endif
      call MPI_Abort(MPI_COMM_WORLD, 77, ier)
      stop 'Arrival-order scheduler: multi-host not supported in v1'
    endif
  enddo
...
!
  ...
  if (ier /= 0) stop 'Error world_bcast_ch'
...
!
  ...
  subroutine irecv_i_inter(recvbuf, recvcount, dest, recvtag, req)
